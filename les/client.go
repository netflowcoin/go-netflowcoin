...
// Package les implements the Light sdvn Subprotocol.
package les
...
func New(stack *node.Node, config *ethconfig.Config) (*Lightsdvn, error) {
...
func (s *Lightsdvn) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *Lightsdvn) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *Lightsdvn) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the sdvn package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *Lightsdvn) APIs() []rpc.API {
...
func (s *Lightsdvn) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *Lightsdvn) BlockChain() *light.LightChain      { return s.blockchain }
func (s *Lightsdvn) TxPool() *light.TxPool              { return s.txPool }
func (s *Lightsdvn) Engine() consensus.Engine           { return s.engine }
func (s *Lightsdvn) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *Lightsdvn) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *Lightsdvn) EventMux() *event.TypeMux           { return s.eventMux }

// Protocols returns all the currently configured network protocols to start.
func (s *Lightsdvn) Protocols() []p2p.Protocol {
...
// light sdvn protocol implementation.
func (s *Lightsdvn) Start() error {

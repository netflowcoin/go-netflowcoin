...
// nodeDockerfile is the Dockerfile required to run an sdvn node.
var nodeDockerfile = `
FROM sdvn/client-go:latest
...
{{end}}
RUN \
  echo 'sdvn --cache 512 init /genesis.json' > sdvn.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.sdvn/keystore/ && cp /signer.json /root/.sdvn/keystore/' >> sdvn.sh && \{{end}}
	echo $'exec sdvn --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> sdvn.sh

ENTRYPOINT ["/bin/sh", "sdvn.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an sdvn node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.sdvn{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.sdvnhash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
// deployNode deploys a new sdvn node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 sdvn --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.sdvn"],
		ethashdir:  infos.volumes["/root/.sdvnhash"],
		port:       port,

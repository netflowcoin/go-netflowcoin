...
Use `devp2p enrdump <base64>` to verify and display an sdvn Node Record.
...
- `-sdvn-network <mainnet/rinkeby/goerli/ropsten>` filters nodes by "sdvn" ENR entry
- `-les-server` filters nodes by LES server support
- `-snap` filters nodes by snap protocol support

For example, given a node set in `nodes.json`, you could create a filtered set containing
up to 20 sdvn mainnet nodes which also support snap sync using this command:

    devp2p nodeset filter nodes.json -sdvn-network mainnet -snap -limit 20
...
### sdvn Protocol Test Suite

The sdvn Protocol test suite is a conformance test suite for the [sdvn protocol][sdvn].

To run the sdvn protocol test suite against your implementation, the node needs to be initialized as such:

1. initialize the sdvn node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run sdvn with the following flags:
```
sdvn --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the sdvn node: 
 ```
 devp2p rlpx sdvn-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```
...
To run the eth66 protocol test suite, initialize a sdvn node as described above and run the following command,
replacing `<enode>` with the enode of the sdvn node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[sdvn]: https://github.com/seaskycheng/devp2p/blob/master/caps/sdvn.md
[dns-tutorial]: https://sdvn.org/docs/developers/dns-discovery-setup

...
// information related to the sdvn protocol such als blocks, transactions and logs.
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(sdvn.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as sdvn.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria sdvn.FilterQuery
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(sdvn.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}

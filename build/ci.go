...
import (
	...
	"github.com/seaskycheng/sdvn/crypto/signify"
	"github.com/seaskycheng/sdvn/internal/build"
	"github.com/seaskycheng/sdvn/params"
)

var (
	// Files that end up in the sdvn*.zip archive.
	sdvnArchiveFiles = []string{
		"COPYING",
		executablePath("sdvn"),
	}

	// Files that end up in the sdvn-alltools*.zip archive.
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert sdvn contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
			Description: "sdvn bootnode.",
		},
		{
			BinaryName:  "evm",
			Description: "Developer utility version of the EVM (sdvn Virtual Machine) that is capable of running bytecode snippets within a configurable environment and execution mode.",
		},
		{
			BinaryName:  "sdvn",
			Description: "sdvn CLI client.",
		},
		{
			BinaryName:  "puppeth",
			Description: "sdvn private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "sdvn account management tool.",
		},
	}
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "sdvnstore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basesdvn = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		sdvn     = "sdvn-" + basesdvn + ext
		alltools = "sdvn-alltools-" + basesdvn + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(sdvn, sdvnArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{sdvn, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with sdvn-release.pub"
...
func doDockerImage(cmdline []string) {
	var (
		upload = flag.String("upload", "", `Where to upload the docker image (usually "sdvn/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
	...
	//  - sdvn/client-go:latest                            - Pushes to the master branch, sdvn only
	//  - sdvn/client-go:stable                            - Version tag publish on GitHub, sdvn only
	//  - sdvn/client-go:alltools-latest                   - Pushes to the master branch, sdvn & tools
	//  - sdvn/client-go:alltools-stable                   - Version tag publish on GitHub, sdvn & tools
	//  - sdvn/client-go:release-<major>.<minor>           - Version tag publish on GitHub, sdvn only
	//  - sdvn/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, sdvn & tools
	//  - sdvn/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, sdvn only
	//  - sdvn/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, sdvn & tools
	var tags []string
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "seaskycheng/sdvn")`)
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "sdvn-ci")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the sdvn sources.
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "sdvn-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "sdvn"
...
type debMetadata struct {
	...
	// sdvn version being built. Note that this
...
func newDebMetadata(distro, goboot, author string, env build.Environment, t time.Time, name string, version string, exes []debExecutable) debMetadata {
	if author == "" {
		...
		author = "sdvn Builds <418012770@qq.com>"
	}
	return debMetadata{
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "sdvn.exe" {
			sdvnTool = file
		} else {
	...
	// first section contains the sdvn binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"sdvn":     sdvnTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.sdvn.nsi", filepath.Join(*workdir, "sdvn.nsi"), 0644, nil)
	...
	installer, _ := filepath.Abs("sdvn-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "sdvn.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archive (usually "sdvnstore/builds")`)
	)
	flag.CommandLine.Parse(cmdline)
	env := build.Env()
	tc := new(build.GoToolchain)
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.sdvn", "-v", "github.com/seaskycheng/sdvn/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("sdvn.aar", filepath.Join(GOBIN, "sdvn.aar"))
		os.Rename("sdvn-sources.jar", filepath.Join(GOBIN, "sdvn-sources.jar"))
		return
	}
	...
	archive := "sdvn-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("sdvn.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/seaskycheng/sdvn/mobile")
	...
	// Create the archive.
	maybeSkipArchive(env)
	archive := "sdvn-" + archiveBasename("ios", params.ArchiveVersion(env.Commit))
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "sdvn.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "sdvn.podspec", "--allow-warnings")
	}
}
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "sdvnstore/builds")`)

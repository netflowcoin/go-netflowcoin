...
// Package accounts implements high level sdvn account management.
package accounts
...
// Account represents an sdvn account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // sdvn account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain sdvn.ChainStateReader)
...
//   keccak256("\x19sdvn Signed Message:\n"${message length}${message}).
//
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19sdvn Signed Message:\n%d%s", len(data), string(data))

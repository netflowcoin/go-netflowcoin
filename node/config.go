...
import (
	...
	"github.com/seaskycheng/sdvn/accounts/external"
	"github.com/seaskycheng/sdvn/accounts/keystore"
	"github.com/seaskycheng/sdvn/accounts/scwallet"
	"github.com/seaskycheng/sdvn/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of sdvn is "sdvn". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "sdvn", keep that.
	if name == "sdvn" || name == "sdvn-testnet" {
		name = "sdvn"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "sdvn" instances.
var isOldsdvnResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by sdvn 1.4 are used if they exist.
	if warn, isOld := isOldsdvnResource[path]; isOld {
		oldpath := ""
		if c.name() == "sdvn" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldsdvnResourceWarning, "Using deprecated resource file %s, please move this file to the 'sdvn' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "sdvn-keystore")
		ephemeral = keydir
	}
